...
func setLogByEcho(span opentracing.Span, c echo.Context) {
	paramNameM := map[string]string{}
	paramsName := c.ParamNames()
	paramsValue := c.ParamValues()
...
func setError(span opentracing.Span, err *echo.HTTPError) {
	isError := err.Code > http.StatusNoContent && err.Code != http.StatusConflict
